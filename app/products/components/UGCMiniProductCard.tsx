...
export default function UGCMiniProductCard({ product, selectedSize, onSelectSize }: UGCMiniProductCardProps) {
  ...
  return (
    <Link href={product.href} className="group flex flex-col rounded-lg border border-ink/10 bg-white p-2.5">
      ...
      <div className="flex flex-1 flex-col gap-1 pt-2.5">
        ...
        <span className="text-[11px] font-semibold uppercase leading-tight tracking-tight text-ink line-clamp-2 min-h-[2lh]">{product.title}</span>
        ...
        {hasSizes && (
          <div className="flex flex-wrap gap-1 pt-1.5">
            {product.sizes.map((size) => {
              ...
              return (
                <button key={size} type="button" disabled={out} onClick={(e) => selectSize(e, size)} className={`px-2 py-1 text-[10px] md:px-1.5 md:py-0.5 md:text-[8px] font-mono tracking-eyebrow uppercase border transition-colors duration-fast ${out ? "border-ink/10 text-ink-faint line-through cursor-not-allowed" : selected ? "border-ink bg-ink text-fg" : "border-ink/20 text-ink-subtle hover:border-ink hover:text-ink"}`}>
                  {size}
                </button>
              );
            })}
